...
| `faster-whisper` INT8 | Implementado localmente | Backend intercambiable en `whisper_service.py` (`WHISPER_BACKEND=faster` por defecto, degrada a `openai` si la librería falta). No instalable en este entorno de desarrollo: falta medir WER rural, latencia y RAM en 1 vCPU/6 GB real (#215) antes de darlo por confirmado |
...
2. benchmark completo en 1 vCPU/6 GB (#215), incluida la latencia real de
   `faster-whisper` y de la síntesis TTS en paralelo (medidas solo en
   hardware de desarrollo hasta ahora, no en el piso ni en el VPS);
