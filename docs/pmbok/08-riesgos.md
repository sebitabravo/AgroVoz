...
## 2. Evidencia del corte

| Evidencia | Resultado | Lectura de riesgo |
|---|---|---|
| Arquitectura y constraints en `AGENTS.md` | Open-WA, un VPS, SQLite, request síncrono y piso de 1 vCPU / 6 GB | Dependencias y cuellos de botella explícitos |
...
| R09 | Retenciones y logs pueden contener datos seudonimizados → borrado incompleto o filtración → daño al titular y hallazgo legal | Alta | Alta | Solicitud de supresión que no cubre todas las copias o log sensible | Evitar contenido sensible en logs; definir TTL y borrado E2E | Tests de privacidad y gates presentes; cobertura integral pendiente | **Por designar** |
| R10 | `success_rate` no equivale a entrega efectiva → panel informa éxito aunque falle LLM/envío → decisiones operativas incorrectas | Alta | Media | Diferencia entre intent clasificado y entrega confirmada | Corregir definición y validar contra estado de entrega | Deuda conocida en `AGENTS.md`; sin cierre verificado | Liderazgo técnico + Product Owner |
...
|---|---|---|---|
| I01 | Resuelto al corte | La suite completa tenía 6 fallos en el worktree | 1.639 aprobados, 0 fallos y 25 omitidos tras las correcciones |
...
aceptación o cierre formal de los riesgos anteriores.
