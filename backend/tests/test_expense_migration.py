"""Pruebas de la migración de gastos y consentimiento específico."""
...
_PREVIOUS_REVISION = "f2a8c1d7e4b6"
_CURRENT_REVISION = "a7c4e9b2d1f6"
...
def test_upgrade_crea_consentimiento_y_gastos_acotados(tmp_path: Path) -> None:
    """Legacy queda sin opt-in y la tabla aplica integridad y cascada."""
    database_path = tmp_path / "expenses_upgrade.db"
    ...
    columns = {
        column["name"]: column
        for column in inspector.get_columns("user_prefs")
    }
    assert columns["expense_consent"]["nullable"] is False
    assert columns["expense_consent"]["default"] is not None

    with engine.begin() as connection:
        legacy_consent = connection.execute(
            text(
                """
                SELECT expense_consent
                FROM user_prefs
...
def test_downgrade_elimina_feature_y_conserva_preferencias(
    tmp_path: Path,
) -> None:
    """El downgrade quita tabla/consentimiento sin borrar la identidad."""
    database_path = tmp_path / "expenses_downgrade.db"
    config = _alembic_config(database_path)
    engine = _engine(database_path)
    ...
    inspector = inspect(engine)
    assert "expenses" not in inspector.get_table_names()
    columns = {
        column["name"]
        for column in inspector.get_columns("user_prefs")
    }
    assert "expense_consent" not in columns
    with engine.connect() as connection:
        preserved = connection.execute(
            text(
                """
                SELECT phone_hash, dataset_consent
                FROM user_prefs
                WHERE phone_hash = :phone_hash
                """
            ),
