...
class TestUserAdminAuth:
    ...
    async def test_key_invalido_retorna_401(self, client: AsyncClient) -> None:
        resp = await client.get(
            f"/api/v1/admin/users/{_VALID_HASH}", headers={"X-Admin-Key": "x"}
        )
        assert resp.status_code == 401
...
class TestSetComuna:
    ...
    async def test_actualiza_comuna_existente(
    ...
    async def test_idempotente_mismo_put_dos_veces(
    ...
    async def test_phone_hash_invalido_retorna_422(
        self, client: AsyncClient
    ) -> None:
    ...
    async def test_comuna_con_espacios_se_strippeda(
        self, client: AsyncClient
    ) -> None:
...
class TestGetUserPrefs:
    ...
    async def test_retorna_prefs_existente(
        self, client: AsyncClient, tmp_path: Path
    ) -> None:
        ...
        resp = await client.get(
            f"/api/v1/admin/users/{_VALID_HASH}", headers=_ADMIN_HEADERS
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["phone_hash"] == _VALID_HASH
    ...
    async def test_phone_hash_invalido_retorna_422(
        ...
        resp = await client.get(
            f"/api/v1/admin/users/{_INVALID_HASH}", headers=_ADMIN_HEADERS
        )
        assert resp.status_code == 422
...
class TestPrivacidad:
    ...
    async def test_respuesta_no_contiene_numero_claro(
        self, client: AsyncClient
    ) -> None:
    ...
    async def test_phone_hash_en_path_no_se_refleja_en_claro(
        self, client: AsyncClient
    ) -> None:
