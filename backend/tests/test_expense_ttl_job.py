"""Regresiones del enforcement TTL de gastos: job CLI y scheduler (#170).
...
from app.jobs.purge_expenses import main as purge_job_main
from app.main import (
    _EXPENSE_PURGE_INTERVAL_SECONDS,
    _cancel_background_task,
    _expense_purge_scheduler,
    _start_expense_purge_scheduler,
)
from app.services.expense_service import ExpenseOperationError


class TestPurgeExpensesJob:
    ...
    def test_exit_cero_cuando_la_purga_confirma(self) -> None:
        ...
        with patch(
            "app.jobs.purge_expenses.purge_expired_expenses",
            return_value=4,
        ) as purge:
    ...
    def test_exit_uno_si_operacion_no_se_confirma(self) -> None:
        ...
        with patch(
            "app.jobs.purge_expenses.purge_expired_expenses",
            side_effect=ExpenseOperationError("fallo"),
        ):
            exit_code = purge_job_main([])
...
class TestExpensePurgeScheduler:
    ...
    async def test_ejecuta_en_thread_y_espera_24_horas(self) -> None:
        ...
        with (
            ...
            await _expense_purge_scheduler()

        assert to_thread.await_count == 1
        sleep.assert_awaited_once_with(_EXPENSE_PURGE_INTERVAL_SECONDS)

    @pytest.mark.asyncio
    async def test_continua_despues_de_error_operativo(self) -> None:
        """Una purga fallida no mata las ejecuciones diarias siguientes."""
        to_thread = AsyncMock(side_effect=[ExpenseOperationError("fallo"), 0])
    ...
    async def test_gate_apagado_igual_crea_la_tarea(self) -> None:
        ...
        with (
            ...
            patch("app.main._expense_purge_scheduler", controlled_scheduler),
        ):
            task = _start_expense_purge_scheduler()
            await started.wait()
